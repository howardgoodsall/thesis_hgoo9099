...
class USPS(data.Dataset):
    ...
    def __init__(self, root, scale_factor=1.0, train=True, transform=None, download=False):
        """Init USPS dataset."""
        # init params
        ...
        self.train_data, self.train_labels = self.load_samples(scale_factor=scale_factor)
        if self.train:
    ...
    def load_samples(self, scale_factor=1.0):
        ...
        if self.train:
            ...
            data_set = data_set[:int(self.dataset_size*scale_factor)]
            images = data_set[0][0]
            labels = data_set[0][1]
            self.dataset_size = labels.shape[0]
        else:
...
class USPS_idx(data.Dataset):
    ...
    def __init__(self, root, scale_factor=1.0, train=True, transform=None, download=False):
        ...
        self.train_data *= 255.0
        self.train_data = np.squeeze(self.train_data).astype(np.uint8)
        print("USPS size: " + str(len(self.train_data)))
