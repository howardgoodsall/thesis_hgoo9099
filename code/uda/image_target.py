...
def train_target_rot(args):
    ...
    while iter_num < max_iter:
        ...
        r_labels_target = torch.from_numpy(r_labels_target).type(torch.LongTensor)#Cast from numpy arr to torch longtensor
        r_inputs_target = rotation.rotate_batch_with_labels(inputs_test, r_labels_target)
        r_labels_target = r_labels_target.cuda()
        r_inputs_target = r_inputs_target.cuda()
        
        f_outputs = netB(netF(inputs_test)).cuda()
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if not args.ssl == 0:
            r_labels_target = np.random.randint(0, 4, len(inputs_test))
            r_inputs_target = rotation.rotate_batch_with_labels(inputs_test, r_labels_target)
            r_labels_target = torch.from_numpy(r_labels_target).type(torch.LongTensor)
            r_labels_target = r_labels_target.cuda()
            r_inputs_target = r_inputs_target.cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='office')#choices=['VISDA-C', 'office', 'office-home']
